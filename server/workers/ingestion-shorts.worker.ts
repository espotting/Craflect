...
import { shortsIngestionQueue, transcriptionQueue } from './scheduler';
...
async function scrapeVideos(zone: any, niche: string): Promise<any[]> {
  ...
  if (keywords.length === 0) {
    console.log(`[scrapeVideos-shorts] No keywords for niche: ${niche}`);
    return [];
  }

  console.log(`[scrapeVideos-shorts] Starting Apify YouTube scrape — niche: ${niche}, keywords: ${keywords.slice(0, 3).join(', ')}...`);
  ...
  try {
    ...
    console.log(`[scrapeVideos-shorts] Apify run completed — runId: ${run.id}, status: ${run.status}`);

    const { items } = await apify.dataset(run.defaultDatasetId).listItems();

    console.log(`[scrapeVideos-shorts] Retrieved ${items.length} items from dataset`);

    return items.map((item: any) => ({
      platform: 'shorts',
      platformVideoId: item.id || `shorts_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`,
      videoUrl: item.url || (item.id ? `https://youtube.com/shorts/${item.id}` : null),
      thumbnailUrl: item.thumbnailUrl || item.bestThumbnail?.url || null,
      followersCount: item.channelSubscriberCount || null,
      ...
      shares: 0,
      creatorName: item.channelName || item.channel || 'unknown',
      creatorPlatformId: item.channelId || null,
      creatorUrl: item.channelUrl || null,
    ...
    console.error(`[scrapeVideos-shorts] Apify error for niche ${niche}:`, error.message || error);
    return [];
  }
}

export const shortsIngestionWorker = new Worker('ingestion-shorts', async (job) => {
  if (job.name === 'cycle-zones') {
    console.log(`[Ingestion-Shorts] 🔄 cycle-zones triggered — dispatching zone+niche jobs...`);
    ...
    if (US_ONLY_MODE) {
      console.log(`[Ingestion-Shorts] 🇺🇸 US-ONLY mode — ${allActiveZones.length - activeZones.length} zones paused`);
    }
    let dispatched = 0;

    for (const zoneCode of activeZones) {
      for (const niche of ALL_NICHES) {
        await shortsIngestionQueue.add('scrape-zone-niche', {
          zoneCode,
          niche,
        }, {
          priority: 2,
    ...
    console.log(`[Ingestion-Shorts] ✅ Dispatched ${dispatched} jobs (${activeZones.length} zones × ${ALL_NICHES.length} niches)`);
  ...
  if (!zoneCode || !niche) {
    console.warn(`[Ingestion-Shorts] ⚠️ Missing zoneCode or niche in job data:`, job.data);
    return;
  }

  console.log(`[Ingestion-Shorts] Processing — Zone: ${zoneCode}, Niche: ${niche}`);

  const zone = await db.query.geoZones.findFirst({
    where: eq(geoZones.zoneCode, zoneCode)
  });

  if (!zone) { console.log(`[Ingestion-Shorts] Zone ${zoneCode} not found`); return; }
  if (!zone.isActive) {
    console.log(`[Ingestion-Shorts] Zone ${zoneCode} inactive, skipping`);
    return;
  }
  ...
  for (const videoData of scrapedVideos) {
    ...
    // Keep only real Shorts (≤ 60s)
    if ((videoData.durationSeconds || 0) > 60) {
      filtered++;
      continue;
    }

    // Lower view threshold for Shorts
    ...
    try {
      const [inserted] = await db.insert(videos).values({
        platform: videoData.platform || 'shorts',
      ...
      console.error(`[Ingestion-Shorts] ❌ Insert failed for ${videoData.platformVideoId}: ${insertError.message}`);
      filtered++;
    }
  }

  console.log(`[Ingestion-Shorts] ${zoneCode}/${niche}: ${created} créées, ${filtered} filtrées, ${duplicates} doublons, ${skippedCreatorLimit} skip créateur, ${skippedTopicLimit} skip topic`);
...
shortsIngestionWorker.on('failed', (job, err) => {
  console.error(`[Ingestion-Shorts] ❌ Job failed: ${job?.name} — ${err.message}`);
});
