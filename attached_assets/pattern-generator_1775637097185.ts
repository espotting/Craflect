...
export async function generatePatternFromCluster(clusterId: string): Promise<boolean> {
  try {
    ...
    // Récupérer les exemples de vidéos du cluster
    ...
    // Sauvegarder dans la table patterns
...
export async function generateAllPatterns(): Promise<number> {
  ...
  // Récupérer les clusters non encore analysés avec assez de vidéos
  ...
  for (const cluster of clusters.rows as any[]) {
    ...
    // Pause pour éviter de saturer l'API OpenAI
